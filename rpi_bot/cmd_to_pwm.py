...
class Velocity_Subscriber(Node):
    ...
    def cmd_to_pwm_callback(self, msg):
        ...
        left_vel = self.speed * ((msg.linear.x - msg.angular.z * self.wheel_separation / 2.0) / self.wheel_radius)
        #right_vel = self.speed * msg.linear.x + self.differential * msg.angular.z
        right_vel = self.speed * ((msg.linear.x + msg.angular.z * self.wheel_separation / 2.0) / self.wheel_radius)
